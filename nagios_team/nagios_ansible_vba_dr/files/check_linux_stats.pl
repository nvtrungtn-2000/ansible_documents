...
sub check_mem {
	...
	if(defined($stat->memstats)) {
		...
		my $swapcached = 0;
		#duhd
		my $memtotal = sprintf("%.2f", $mem->{memtotal}/1048567);
		if($mem->{swaptotal}>0) {
		...
		print "MEMORY $status : Mem used: $memused%, Swap used: $swapused%, Total: $memtotal"."GB"."  $perfdata";
	}
	else {
		print "No data";
	}
		...
		if(!defined($o_pattern)){ $o_pattern = 'all';}
			...
			foreach my $device (keys (%$disk)) {
				...
				if(defined($checkthis->{$mountpoint})||defined($checkthis->{all})){
					...
					if($o_unit =~ /\%/) {
					#duhd
					$perfdata .= " $mountpoint=$percentused".'%'.";$o_warning;$o_critical;0;100";
					...
					my $tmp_warn = sprintf("%.2f", ($o_warning/100)*$total);
                    my $tmp_crit = sprintf("%.2f", ($o_critical/100)*$total);
					...
					if($o_unit =~ /MB/i) {
						...
						$tmptotal = sprintf("%.2f", ($total/1024));
  					    #duhd
						$tmp_warn = sprintf("%.2f", ($o_warning/100)*$tmptotal);
                        $tmp_crit = sprintf("%.2f", ($o_critical/100)*$tmptotal);
					}
					elsif($o_unit =~ /GB/i) {
			...
			#duhd
					...
					$perfdata .= " $mountpoint=$tmpusage$o_unit;$tmp_warn;$tmp_crit;0;$tmptotal";
				}
			}
		}
...
sub check_io {
	...
	if(defined($stat->diskstats)) {
		...
		if(!defined($o_pattern)){ $o_pattern = 'all';}
			...
			foreach my $device (keys (%$disk)) {
				...
				if(defined($checkthis->{$device})||defined($checkthis->{all})){
					if($o_unit =~ /BYTES/i) {#duhd
						if(($rdbyt>=$read_crit)||($wrtbyt>=$write_crit)){ $crit++; $msg=":".$device."_read=".bytes_to_megabytes($rdbyt)."MBps ".$device."_write=".bytes_to_megabytes($wrtbyt)."MBps";}
						elsif(($rdbyt>=$read_warn)||($wrtbyt>=$write_warn)){ $warn++; $msg=":".$device."_read=".bytes_to_megabytes($rdbyt)."MBps ".$device."_write=".bytes_to_megabytes($wrtbyt)."MBps";}

							$perfdata .= ""
								.$device."_read=$rdbyt"."b;$read_warn;$read_crit "
								.$device."_write=$wrtbyt"."b;$write_warn;$write_crit";
					}
					elsif($o_unit =~ /MB/i) {#duhd
						if(($rdbyt/1048576>=$read_crit)||($wrtbyt/1048576>=$write_crit)){ $crit++; $msg=":".$device."_read=".bytes_to_megabytes($rdbyt)."MBps ".$device."_write=".bytes_to_megabytes($wrtbyt)."MBps";}
						elsif(($rdbyt/1048576>=$read_warn)||($wrtbyt/1048576>=$write_warn)){ $warn++; $msg=":".$device."_read=".bytes_to_megabytes($rdbyt)."MBps ".$device."_write=".bytes_to_megabytes($wrtbyt)."MBps";}
						$perfdata .= ""
							.$device."_read=".bytes_to_megabytes($rdbyt)."MB;$read_warn;$read_crit "
							.$device."_write=".bytes_to_megabytes($wrtbyt)."MB;$write_warn;$write_crit";
					}
					else {
		...
		print "DISK IO $status$msg |$perfdata";	
	}
	else {
		print "No data";
	}
